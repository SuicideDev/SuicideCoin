...
#if defined(NDEBUG)
# error "Suicide cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xce2f9ef4aaa93c65e29d0c2df9c551b92ac5b985e6de1aeb6df3d248409d21e5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Suicide: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Suicide Signed Message:\n";
...
{
    // Suicide: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Suicide: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Suicide: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xce2f9ef4aaa93c65e29d0c2df9c551b92ac5b985e6de1aeb6df3d248409d21e5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "What the hell are you guys doing ! 800k suicides in 2012 ?!? Holy crap !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0420ff87425e137cbde86c336f6fb11b0322f36b66376de7652f213679ae2bdc8bcf1062a9ff02bfc44676aa316adcce489585e6b6f2c81015059993966ade21d8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1457131580;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1347457;
        ...
        assert(block.hashMerkleRoot == uint256("0x1b399140bbbf3ca138e82199362604759d7d967ebb8582a18151a74a38d58c35"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x1a, 0xfa, 0xe9, 0x10 }; // Suicide: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuicideMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuicideMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuicideMiner(CWallet *pwallet)
{
    printf("SuicideMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Suicide-miner");
    ...
    try { loop {
        ...
        printf("Running SuicideMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuicideMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuicideMiner, pwallet));
}
